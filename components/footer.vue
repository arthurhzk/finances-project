<template>
  <div>
    <div class="max-w-2xl mx-auto dark:text-slate-200 py-10">
      <div class="text-center">
        <h3 class="text-3xl mb-3">Faça o download do nosso aplicativo</h3>
        <p>WiseHub - Soluções em Finanças</p>
        <div class="flex justify-center my-10">
          <div class="flex items-center border rounded-lg px-4 py-2 w-52 mx-2">
            <img
              src="https://cdn-icons-png.flaticon.com/512/888/888857.png"
              class="w-7 md:w-8"
            />
            <div class="text-left ml-3">
              <p class="text-sm md:text-base">Google Play</p>
            </div>
          </div>
          <div
            class="flex items-center border w-auto rounded-lg px-4 py-2 mx-2"
          >
            <img
              src="https://cdn-icons-png.flaticon.com/512/888/888841.png"
              class="w-7 md:w-8"
            />
            <div class="text-left ml-3">
              <p class="text-sm md:text-base">App Store</p>
            </div>
          </div>
        </div>
      </div>
      <div
        class="mt-28 flex flex-col md:flex-row md:justify-between items-center text-sm text-gray-400"
      >
        <p class="order-2 md:order-1 mt-8 md:mt-0">
          WiseHub - Todos os direitos reservados.
        </p>
        <div class="order-1 md:order-2">
          <span class="px-2">Sobre Nós</span>
          <span class="px-2 border-l">Contato</span>
          <span class="px-2 border-l">Politicas de Privacidade</span>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts"></script>
